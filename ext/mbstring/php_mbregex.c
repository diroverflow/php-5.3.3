...
static void _php_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAMETERS, OnigOptionType options)
{
	...
	eval = 0;
	{
		...
		if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ZZs|s",
									&arg_pattern_zval,
									&replace_zval,
									&string, &string_len,
		...
		replace = Z_STRVAL_PP(replace_zval);
		replace_len = Z_STRLEN_PP(replace_zval);
	...
	if (eval) {
		pbuf = &eval_buf;
		if (IS_TAINTED((*replace_zval))) {
			description = zend_make_compiled_string_description("mbregex replace tainted" TSRMLS_CC);
		}
		else
		{
