...
static int preg_do_eval(char *eval_str, int eval_str_len, char *subject,
						int *offsets, int count, char **result, int input_tainted TSRMLS_DC)
{
	zval		 retval = {0};			/* Return value from evaluation */
	...
	/* Run the code */
	//if input is tainted, mark retval as tainted
	if (input_tainted) {
		Z_TAINT(retval, TAINT_ALL);
	}
...
PHPAPI char *php_pcre_replace(char *regex,   int regex_len,
							  ...
							  int *result_len, int limit, int *replace_count, int input_tainted TSRMLS_DC)
{
	pcre_cache_entry	*pce;			    /* Compiled regular expression */
	...
	return php_pcre_replace_impl(pce, subject, subject_len, replace_val, 
		is_callable_replace, result_len, limit, replace_count, input_tainted TSRMLS_CC);
}
/* }}} */

/* {{{ php_pcre_replace_impl() */
PHPAPI char *php_pcre_replace_impl(pcre_cache_entry *pce, char *subject, int subject_len, zval *replace_val, 
	int is_callable_replace, int *result_len, int limit, int *replace_count, int input_tainted TSRMLS_DC)
{
	pcre_extra		*extra = pce->extra;/* Holds results of studying */
	pcre_extra		 extra_data;		/* Used locally for exec options */
	int				 exoptions = 0;		/* Execution options */
		...
		if (count > 0 && (limit == -1 || limit > 0)) {
			...
			if (eval) {
				eval_result_len = preg_do_eval(replace, replace_len, subject,
											   offsets, count, &eval_result, input_tainted TSRMLS_CC);
 ...
 */
static char *php_replace_in_subject(zval *regex, zval *replace, zval **subject, int *result_len, int limit, int is_callable_replace, int *replace_count, int input_tainted TSRMLS_DC)
{
	zval		**regex_entry,
				**replace_entry = NULL,
				 *replace_value,
...
static void preg_replace_impl(INTERNAL_FUNCTION_PARAMETERS, int is_callable_replace, int is_filter)
{
	...
	//detect if input is tainted
	if (IS_TAINTED((*replace)) || IS_TAINTED((*subject))) {
		input_tainted = 1;
	}
	...
	if (Z_TYPE_PP(subject) == IS_ARRAY) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_PP(subject), (void **)&subject_entry) == SUCCESS) {
			...
			if ((result = php_replace_in_subject(*regex, *replace, subject_entry, &result_len, limit_val, is_callable_replace, &replace_count, input_tainted TSRMLS_CC)) != NULL) {
		...
		old_replace_count = replace_count;
		if ((result = php_replace_in_subject(*regex, *replace, subject, &result_len, limit_val, is_callable_replace, &replace_count, input_tainted TSRMLS_CC)) != NULL) {
			if (!is_filter || replace_count > old_replace_count) {
