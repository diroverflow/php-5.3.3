...
int zend_startup(zend_utility_functions *utility_functions, char **extensions TSRMLS_DC) /* {{{ */
{
	...
	//Z_TAINT(zval_used_for_init, TAINT_N0); //taint to no
...
//taint mark hashtable
void php_taint_mark_strings(zval *symbol_table, int taint_type TSRMLS_DC) /* {{{ */
{
	zval **ppzval;
	HashTable *ht = Z_ARRVAL_P(symbol_table);
	HashPosition pos = {0};
	//php_printf("in php_taint_mark_strings\n");
    ...
    if (Z_TYPE_PP(ppzval) == IS_ARRAY) {
			php_taint_mark_strings(*ppzval, taint_type TSRMLS_CC);
		} else if (IS_STRING == Z_TYPE_PP(ppzval)) {
			//php_printf("mark string 0x%x:%s\r\n",(*ppzval),Z_STRVAL_PP(ppzval));
			Z_TAINT_PP(ppzval, taint_type);
		}
	}
} /* }}} */

//print php warning info
void php_taint_error(const char *docref TSRMLS_DC, const char *format, ...) /* {{{ */ {
	va_list args;
	va_start(args, format);
