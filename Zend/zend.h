...
struct _zval_struct {
	...
	zend_uint taint;	//taint mark
};

//simulate any GPC input
...
#define TAINT_GET 1
#define TAINT_POST 2
#define TAINT_COOKIE 4
#define TAINT_ALL 7
//mark string as tainted
#define STRING_TAINT_MARK ";stringistainted"

#define IS_TAINTED(pz) ((pz->taint&0xffffff00)==TAINT_MARK)
#define IS_TAINT_TAINTED(taint) ((taint&0xffffff00)==TAINT_MARK)
#define Z_TAINT_P(pz,v) zval_taint_p(pz,v)
#define Z_TAINT_PP(ppz,v) Z_TAINT_P(*(ppz),v)
#define Z_TAINT(z,v) Z_TAINT_P(&(z),v)
...
static zend_always_inline void zval_taint_p(zval* pz, int v) {
	pz->taint=(TAINT_MARK|v);
}
...
ZEND_API void php_taint_mark_strings(zval *symbol_table, int taint_type TSRMLS_DC);
ZEND_API void php_taint_error(const char *docref TSRMLS_DC, const char *format, ...);
