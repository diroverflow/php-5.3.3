...
ZEND_VM_HANDLER(73, ZEND_INCLUDE_OR_EVAL, CONST|TMP|VAR|CV, ANY)
{
	...
	php_printf("in ZEND_INCLUDE_OR_EVAL\r\n");
	...
	switch (Z_LVAL(opline->op2.u.constant)) {
		...
		case ZEND_EVAL: {
				...
				//check if tainted
//				php_printf("inc_filename->taint 0x%x:%d\r\n", inc_filename, inc_filename->taint);
//				if(inc_filename->taint == TAINT_POST)
//				{
